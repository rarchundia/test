  ...
  if(isset($_POST['update'])) {
    if(empty($errors)){
       ...
       $descripcion = remove_junk($db->escape($_POST['descripcion']));
            $sql = "UPDATE file SET filename ='{$name}', description ='{$descripcion}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
     ...
     <div class="panel panel-default">
       <div class="panel-heading">
        <strong>
          ...
          Actualiza <?php echo remove_junk(ucwords($e_user['filename'])); ?>
        </strong>
       </div>
       <div class="panel-body">
          <form method="post" action="edit_file.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
