...
if(isset($_POST['entregado'])){
 ...
 $p_quien_recibe = $_POST['quien_recibe'];
	...
	if($db->query($sql)){
      ...
      echo '<meta http-equiv="Refresh" content="0; url=guia_a.php">';
   }
	    else {
 ...
 $id = $_POST['id'];
  $asigna=$_POST['asigna'];
   ...
   if(empty($errors)){
	   ...
	   $query   = "UPDATE entrega SET id_operador='{$operador}', estatus=2, asignado='{$asigna}', fecha_asignacion='{$date}' WHERE id ='{$id}'";
	   $result = $db->query($query);
               if($result && $db->affected_rows() === 1){
                 $session->msg('s',"Asignado Correctamente. ");
                 redirect('guia_a.php', false);
               } else {
...
case 29:
    ...
    default:
        echo " No Tiene Cargado  Ningún Servicio ";
				 ...
				 <td class="text-center"><strong><br><br><strong><?php echo remove_junk($product['quien_genera']);?> </strong></td>
...
<div id="<?php echo $product['id']?>" class="modal fade" role="dialog">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
       ...
       <form action="guia_a.php" method="post">
  ...
  <input type="text" class="form-control datepicker" id="fecha_documentacion" name="fecha_documentacion" placeholder="Fecha que se Dcumento el Paquete" autocomplete="off">
       ...
       <input type="text" name="id" value="<?php echo remove_junk($product['id']);?>"  >    
				...
				<?php 
					...
					if($product['estatus']==55){
					 ...
					 ?>
					 	<form method="post" action="guia_a.php" class="clearfix">
...
<div id="guia" class="tabcontent">
		<br>
		<form class="was-validated" action="guia_a.php" method="post">
