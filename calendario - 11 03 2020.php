...
if(isset($_POST['agregar_calendario'])){
	...
	//INSERT INTO excel (razon_social, guia, fecha_envio, servicio, incidencia, asesor, activo)  VALUES 
     ...
     $query .=") VALUES (";
     $query .="'{$p_inicio}','{$p_id_user}','{$date}','{$p_titulo}','{$p_tiempo_inicio}','7','{$p_fin}','{$p_tiempo_fin}','{$p_color}'";
     $query .=")";
			...
			$(document).ready(function() {
				$('#calendar').fullCalendar({
					...
					eventClick: function(event) {
						...
						$('#visualizar #notas').text(event.notas);
                        $('#visualizar #resumen').text(event.resumen);
                        /*if(event.cerrada==0){
                        $('#visualizar #notas_cierre').html("<a href='notas_reunion.php?id="+event.id+"'>Agrega</a>");}else{
                         $('#visualizar #notas_cierre').text("Ya se han Agregado Notas");   
                        }*/
					...
					events: [
						<?php foreach ($resultado_eventos as  $registros_eventos): ?>
								{
								id: '<?php echo $registros_eventos['id_cita']; ?>',
								title: '<?php echo $registros_eventos['notas'].' -'.$registros_eventos['contacto']; ?>',
                                    contacto: '<?php echo $registros_eventos['contacto']; ?>',
                                   // color: '<?php echo $registros_eventos['color']; ?>',
                                    cerrada: '<?php echo $registros_eventos['cerrada']; ?>',
                                    notas: '<?php echo $registros_eventos['notas'];
                                    if($registros_eventos['resumen']!='NULL'){
                                    
                                    ?>',
                                    resumen: '<?php echo $registros_eventos['resumen'];} ?>',
...
<style>
 ...
 <div class="row">
  ...
  <div class="col-md-12">
    ...
    <!--<?php foreach ($resultado_eventos as  $registros_eventos): ?>
							<?php echo count_id();?>-<?php echo $registros_eventos['id_cita'].'-'. $registros_eventos['notas']; ?><br>
							 <?php endforeach; ?>
		-->   
			<div id='calendar'></div>
		</div>
		...
		<div class="modal fade" id="cadastrar" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" data-backdrop="static">
			<div class="modal-dialog" role="document">
				<div class="modal-content">
					...
					<div class="modal-body">
						<form class="form-horizontal" method="POST" action="calendario.php">
      ...
      <textarea class="form-control" name="titulo" rows="3" data-emojiable="true" data-emoji-input="unicode" onkeyup="mayus(this);" autofocus placeholder="Titulo do Evento" required></textarea>
           </p>
