...
if(isset($_POST['fecha_entrega'])){
  ...
  $fecha=$_POST['fecha'];
	$fecha2=$_POST['fecha2'];
       ...
       redirect('consulta_entrega.php', false);
   }
	?>   
<?php 
include_once('layouts/header.php');
//$norte_odometro = recoleccion_diaria_odometro();
$products = entrega_rango_fecha($fecha,$fecha2);
    ...
    <div class="col-md-12">
		...
		<form class="clearfix" method="post" action="rango_entrega.php">
            <div class="form-group">
                <div class="input-group">
                  <input type="text" class="datepicker form-control" name="fecha"  autocomplete="off" placeholder="Selecciona Primera Fecha" required title="Selecciona un Rango de Fechas de Entrega Para Generar el reporte" data-toggle="tooltip">
                  <span class="input-group-addon"><i class="glyphicon glyphicon-menu-right"></i></span>
                  
                     <input type="text" class="datepicker form-control" name="fecha2"  autocomplete="off" placeholder="Selecciona Segunda Fecha" required>
										</div>
...
<strong>Estatus Entregas</strong>
        </div>
    ...
    default:
        echo "Aun No Esta Asignado a Ninguna Ruta";
			...
			if($product['estatus']!=0){
				?>
				<center><h2>Entregado</h2></center><br>
			<?php 
			$fecha=strftime("%d %B del %Y a las %I:%M %P", strtotime($product['fecha_entrega']));
						echo $fecha;
