<?php
  $page_title = 'Incidencias Historico';
...
include_once('layouts/header.php');
 $products = incidencias_historico();
...
if(empty($errors)){
   ...
   $date    = make_date();
	...
	$nota_concatenada=$p_nota_server." <p> (".$date."  ".$p_user_name." Incidencia Re-Abierta ) </p>";
	
	$query  = " UPDATE excel SET incidencia='{$nota_concatenada}', estatus ='0' WHERE id='{$p_id_nota}'";
	
	 $result = $db->query($query);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Se Ha Reactivado el Caso Con Exito!!! ");
...
<strong><label title="Solo se ve el  estatus de hasta 5 dias" data-toggle="tooltip">Historico Incidencias</label> <?php 
     ...
     width:100%;">
            ...
            <thead>
              <tr>
				...
				<th class="text-center">REABRIR INCIDENCIA</th>
 ...
 <?php echo remove_junk($product['incidencia']); ?></h3></div>
      <div class="modal-footer">
		...
		<form method="post" action="incidencias_historico.php" class="clearfix">
			<input  type="hidden" id="nota_server" name="nota_server" value=" <?php echo remove_junk($product['incidencia']);?> ">	
			 ...
			 <br><br><button type="submit" name="estatus" class="btn btn-success btn-block">Re-Abrir</button>
			</form>
		</td>
