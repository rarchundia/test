<?php
  $page_title = 'Comprobante de Domicilio';
...
<div class="row">
   ...
   <form method="post"  class="dropzone" action="sube_documentos_ajax.php" id="form_constitutiva" enctype="multipart/form-data">
    <input type="hidden" name="correo" id="correo" value="<?php echo $user["email"]?>">
     ...
     <input type="hidden" name="empresa5" id="empresa5" value="<?php echo $id_varible; ?>" >
	<!-- <input type="text" name="empresa5" id="empresa5" value="<?php echo $id_varible; ?>" >
	-->  <br><br><br>
			</form>
		...
		<?php $url=$_SERVER['HTTP_REFERER']; 
			...
			<?php
							...
							?>
			<a href="edo_cuenta.php?id=<?php echo (int)$id_varible; ?>" class="btn btn-primary" id="btn_siguiente">Siguiente</a>
    ...
    formData.append("empresa5", jQuery('#empresa5').val()); // attach dropzone image element
...
*/
    ...
    dictDefaultMessage: "Arrastra El Archivo Aqui o Da Click.<br>-Documento Comprobante de Domicilio-",
			maxFilesize: 10, // MB
                maxFiles: 1,
			addRemoveLinks: true,
