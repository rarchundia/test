...
if($usuario=="12" or $usuario=="9"){
	$recent_sales = ultimos_contactos_todos();
}else{
...
<strong>Panel
    ...
    box-shadow: 0 1px 1px rgba(229, 103, 23, 0.075)inset, 0 0 8px rgba(255,144,0,0.6);" class="form-control" autofocus placeholder="Buscar por Nombre, Empresa, Dirección... "  id="myInput"  />
     ...
     width:100%">
			...
			<table class="table table-striped table-hover table-responsive" id="pipedrive_table">
				<thead>
					<tr>
						<?php if($usuario=="12" or $usuario=="9"){
						?>
						<th class='text-center'>De</th>
						<?php
}else{?>
						<th class='text-center'>id</th><?php }?>
						<th>Contacto </th>
						<th>Empresa </th>
				...
				<tbody id="myTable">
					<?php foreach($recent_sales as $pipe): ?>
						<tr>
							 
							<td class='text-center'>
								<?php if($usuario=="12" or $usuario=="9"){
						?>
						<small><?php echo $pipe["name"]; ?></small>
						<?php
}else{?>
						<a href="edit_contacto.php?id=<?php echo $pipe["id"]; ?>"> <label class=" glyphicon glyphicon-pencil" title="Editar Contacto" data-toggle="tooltip" ></label></a>
								<?php }?>
								   ...
								   <td class="text-center"><a href="detalle_contacto.php?id=<?php echo $pipe["id"] ?>" title="Ver Historial de Acciones" data-toggle="tooltip"><?php echo $pipe["contacto"];?></a></td>
							
							
							<td >
								
								 <a onclick="window.open('https://www.google.com/search?q=<?php echo    $pipe["empresa"];?>', 'Google Envipaq', 'width=900, height=600'); return false" title="Buscalo en Google" data-toggle="tooltip"><span class="glyphicon glyphicon-search"></span><?php echo    $pipe["empresa"];?>
							...
							<td>
								...
								<a onclick="window.open('https://maps.google.com/?q=<?php echo    $pipe["domicilio"];?>', 'Maps Envipaq', 'width=900, height=600'); return false" title="Buscalo en el  Mapa" data-toggle="tooltip">
				...
				<span class="glyphicon glyphicon-map-marker"></span>Ubicalo en el Mapa</a>
							...
							<td class='text-center' >
								...
								<a href="#" >
									...
									<label class=" glyphicon glyphicon-plus-sign"  title="Agregar Acción" data-toggle="modal" data-target="#accion<?php echo    $pipe["id"];?>" style="font-size:20px;"></label></a>
								
								<!-- Modal -->
<div id="accion<?php echo    $pipe["id"];?>" class="modal fade" role="dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="close" data-dismiss="modal">&times;</button>
           <h4 class="modal-title" id="exampleModalLabel"><strong><center>Agendar Cita con <?php echo $pipe["contacto"]; ?> de la empresa  <?php echo $pipe["empresa"]; ?></center>
      ...
      <div class="modal-body">
		...
		<form method="post" action="pipedrive.php">
  			<input type="hidden"class="form-control" name="id_user" value="<?php echo $usuario ?>">
			<input type="hidden" class="form-control" name="id_contacto" value="<?php echo $pipe["id"]; ?>">
		...
		<div class="col-md-12">	
			<label for="level"><br>Fecha y Hora</label><br></div>
			<div class="col-md-5">	
  <div class="form-group">
...
<br><br>
  ...
  <div class="form-group"><br>
    ...
    <textarea class="form-control" name="notas" rows="3" onkeyup="mayus(this);" placeholder="Escribe Aqui tus notas si tienes "></textarea>
  </div></div>
